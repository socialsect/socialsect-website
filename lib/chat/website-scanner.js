...
export async function scanWebsite(url) {
  try {
    ...
    // Fetch PageSpeed metrics and HTML in parallel to reduce latency
    console.log('[SCANNER] Fetching PageSpeed metrics and HTML in parallel...')
    const [pageSpeedData, htmlData] = await Promise.all([
      fetchPageSpeedMetrics(fetchUrl).catch(err => {
        console.error('[SCANNER] PageSpeed fetch failed, continuing with static checks:', err.message)
        return { metrics: {}, score: null }
      }),
      fetchWebsiteHtml(fetchUrl)
    ])
