...
export function resourceDownloadConfirmationEmail({ resourceTitle }) {
  return buildBrandedEmail({
    icon: 'file',
    eyebrow: 'Request received',
    headline: 'We\u2019re sending your resource shortly.',
    greeting: 'Hi there,',
    paragraphs: [
      `Thanks for requesting <strong>${escapeHtml(resourceTitle)}</strong>.`,
      'A member of our team will email it to you shortly. No spam. No drip campaign \u2014 just the material you asked for.',
...
export function newsletterConfirmationEmail({ source, name }) {
...
export function productDemoConfirmationEmail({ name, demoOption }) {
  const firstName = name.split(/\s+/)[0] || name
  return buildBrandedEmail({
    icon: 'calendar',
    eyebrow: 'Request received',
    headline: 'Were reviewing your demo request.',
    greeting: `Hi ${firstName},`,
    paragraphs: [
      `Thanks for asking to see <strong>${escapeHtml(demoOption)}</strong>.`,
      'A member of our team will review your details personally and reply within <strong>24 hours</strong> to schedule the best walkthrough.',
      'No automated drip. No extra marketing. Just the next step for your practice.',
    ],
    signOff: 'Speak soon,',
  })
}
