...
export async function sendVisitorConfirmation({ to, subject, html }) {
  ...
  // In test mode with onboarding@resend.dev, only send to verified account email
  const isTestMode = fromEmail === 'onboarding@resend.dev'
  
  if (isTestMode) {
    const verifiedEmail = 'vinayakxsingh21@gmail.com'
    if (to.toLowerCase() !== verifiedEmail.toLowerCase()) {
      console.warn(`[${new Date().toISOString()}] ⚠️  Test mode: Cannot send to ${to}. Only ${verifiedEmail} allowed with onboarding@resend.dev`)
      console.log(`[${new Date().toISOString()}] Skipping visitor confirmation email in test mode`)
      // Return a mock success response to avoid breaking the flow
      return { id: 'test-mode-skipped', message: 'Email skipped in test mode' }
    }
  }
