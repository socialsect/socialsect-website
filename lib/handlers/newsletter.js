...
export async function processNewsletter(payload = {}, requestMeta = {}) {
  ...
  const label = name ? `${escapeHtml(name)} (${escapeHtml(email)})` : `<strong>${escapeHtml(email)}</strong>`
  ...
  const teamHtml = `<div style="font-family:Inter,Arial,sans-serif;font-size:15px;line-height:1.5;color:#1A1C1D"><p style="margin:0 0 24px">${label} subscribed to the newsletter.</p><table style="border-collapse:collapse;width:100%;max-width:560px">${table}</table></div>`
  ...
  return finalizeSubmission({
    ...
    sendTeamEmail: () =>
      sendNotificationEmail({
        subject: `Newsletter signup${name ? `  ${name}` : ''} (${source})`,
        replyTo: email,
        html: teamHtml,
      }),
    sendVisitorEmail: () =>
      sendVisitorConfirmation({
        ...
        html: newsletterConfirmationEmail({ source, name }),
      }),
  })
}
