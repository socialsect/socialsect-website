...
export default function IntroLoader() {
  ...
  const overlayClassName = [
    'intro-loader',
    isReduced ? 'intro-loader--reduced' : '',
    isExiting ? 'intro-loader--exiting' : '',
  ]
    .filter(Boolean)
    .join(' ')
  ...
  return (
    <div className={overlayClassName} aria-hidden="true">
      <div className="intro-loader__logoWrap">
        <svg
          className="intro-loader__svg"
        ...
        >
          {LOGO_PATHS.map((d, index) => (
            <path
              ...
              d={d}
              pathLength="1"
              style={{ animationDelay: `${index * 0.18}s, ${2.15 + index * 0.18}s` }}
            />
          ))}
        </svg>
      </div>
