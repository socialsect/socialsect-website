...
void main() {
  ...
  // Remove the hueShift application to base color
  // col.rgb = hueShiftRGB(col.rgb, uHueShift); ← remove this
  ...
  // Brand color: #695af2
  vec3 brand = vec3(0.412, 0.353, 0.949);

  // Blend: white bg + brand-tinted animation
  float lum = dot(col.rgb, vec3(0.299, 0.587, 0.114));
  col.rgb = mix(vec3(1.0), brand, lum * uBlend);

  gl_FragColor = vec4(clamp(col.rgb, 0.0, 1.0), 1.0);
}
`;

function logShaderErrors(gl, program, vertexSrc, fragmentSrc) {
  ...
  if (vertLog) console.error('DarkVeil vertex shader:', vertLog);
  if (fragLog) console.error('DarkVeil fragment shader:', fragLog);
  if (linkLog) console.error('DarkVeil program link:', linkLog);

  if (!gl.getShaderParameter(program.vertexShader, gl.COMPILE_STATUS)) {
    console.error('DarkVeil vertex source:\n', vertexSrc);
  }
  if (!gl.getShaderParameter(program.fragmentShader, gl.COMPILE_STATUS)) {
    console.error('DarkVeil fragment source:\n', fragmentSrc);
  }
}
...
export default function DarkVeil({
  ...
  useEffect(() => {
    ...
    if (!gl.getProgramParameter(program.program, gl.LINK_STATUS)) {
      logShaderErrors(gl, program, vertex, fragment);
      return undefined;
    }
